...
function LandingPage() {
    ...
    const { data: latestMixes } = useGetLatestMusicQuery(1);
            ...
            <div className="mx-4 w-full my-6">
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 xs:grid-cols-5 gap-1">
                        {trendingMixes?.map((tmix: any) => (
                            <MusItem key={tmix.id} plays='32K' title={tmix?.owner.username} />
                        ))}
                    </div>
                </div>
                <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 xs:grid-cols-5 gap-2">
                        {latestMixes?.map((mix: any) => (
                            <EasyAfterNoon key={mix.id} image={mix.video_thumbnail} />
                        ))}
                    </div>
                </div>
            </div>

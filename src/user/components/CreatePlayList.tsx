...
import { useCreatePlayListMutation } from '../UsersState';
const schema = yup.object({
    playlist_name: yup.string().required(),
    playlist_description: yup.string().required(),
    playlist_type: yup.string().required(),
    // playlist_thumbnail: yup.string().required(),
}).required()
interface IPlayList{
...
export default function CreatePlayListModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void}) {
    const [isSubmitting, setIsSubmitting] = useState<boolean>(false);
    const { handleSubmit, register,formState:{errors} } = useForm<IPlayList>({
        resolver: yupResolver(schema),
    })
    const [createPlaylist,] = useCreatePlayListMutation();
    if (!isOpen) return null;
    const onSubmit: SubmitHandler<IPlayList> = async (data: IPlayList) => {
        setIsSubmitting(!isSubmitting);
        const userData = {
            playlist_name: data.playlist_name,
            playlist_description: data.playlist_description,
            playlist_type: data.playlist_type,
            playlist_thumbnail: "https://picsum.photos/200/300",
        }
        console.log(userData);
        try {
            const response = await createPlaylist(userData).unwrap();
                                    ...
                                    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col mr-5 ml-5">
                                        ...
                                        <div className="mb-3">
                                            <input
                                                ...
                                                required
                                            />
                                            <p className="text-red-500 text-xs italic">{errors.playlist_name?.message}</p>
                                        </div>
                                        <div className="mb-3">
                                            {/* <input
                                                type="text"
                                                {...register('playlist_type')}
                                                id="playlist_name"
                                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                placeholder="Playlist Name"
                                                required
                                            ...
                                            <select
                                                ...
                                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            >
                                                <option value="" label="Select Playlist Type" />
                                                {Array.isArray(playListType) && playListType.map((route,id) => (

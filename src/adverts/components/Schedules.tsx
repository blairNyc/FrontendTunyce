  ...
  useEffect(() =>{
    ...
    const fetchData = async () => {
      try {
        ...
        const responseRoute = await axios.get('https://warm-journey-18609535df73.herokuapp.com/api/v1/adverts/schedules/', {
          headers: {
    ...
    <div className="bg-white p-4 rounded-lg">
      ...
      <div className="mt-4 w-full flex flex-col gap-2">
        {schedules?.map((schedule) => (
          <div key={schedule.id} className='w-full flex flex-row space-x-10 p-1 gap-2 border-b justfy-center align-center'>
            <img
            ...
            <p className='mt-2'>{schedule.advert.title} {schedule.slot.name}</p>
            {/* <p className='mt-2'>{schedule.cost}</p> */}
          </div>
        ))}
